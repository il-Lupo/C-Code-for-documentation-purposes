...
char* Return_a_string_from_a_function()
{
    ...
    // hier verarbeiten
    size_t bytesize = sizeof(char) * (size_t)21;
...
{
    memset(indexes, (short)-1, sizeof(short) * max);
...
void Say_Programme_ends_with_frame()
{
    ...
    //      ║                                                         ║
    printf("\xBA \bProgramm wird beendet. Dr\x81 \bcken Sie 'Enter'.\xBA\n");
    putchar('\xC8'); // ╚
    for (int i = 0; i < 43; i++)
    {
        putchar('\xCD'); // ═
...
{
    wchar_t buffer[PathLength] = { '\0' };
    GetModuleFileName(NULL, buffer, PathLength);
    int cnt1 = 0;
    ...
    size_t bytesize = sizeof(wchar_t) * cnt1 + sizeof(wchar_t);
    wchar_t* StrOut = malloc(bytesize);
    if (StrOut == NULL)
    {
        printf("Insufficient memory!\n");
        return NULL;
    }
    memset(StrOut, 0, bytesize);
